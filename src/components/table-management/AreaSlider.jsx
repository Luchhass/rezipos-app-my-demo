...
export default function AreaSlider({ selectedArea, setSelectedArea, activeAction }) {
  ...
  const handleMouseMove = (e) => {
    ...
    sliderRef.current.scrollLeft =
      scrollLeft.current - (e.pageX - sliderRef.current.offsetLeft - startX.current);
  };

  const stopDragging = () => (isDown.current = false);

  // Normal kullanımda quick order area gizlenir
        ...
        {isLoading ? (
          <>
            ...
            {[...Array(9)].map((_, i) => (
              <div
                key={i}
                className="flex flex-col justify-between w-40 h-32.5 p-4 rounded-2xl shrink-0 animate-pulse bg-[#f4efc4] md:w-42.5 md:h-34 md:p-5 lg:w-45 lg:h-38 lg:p-6"
              >
                <span />
                <div className="text-left">
                  <div className="w-20 h-4 mb-2 rounded-lg bg-black/10 dark:bg-white/10" />
          ...
          <>
            ...
            <button
              ...
              className="flex flex-col justify-between w-40 h-32.5 p-4 rounded-2xl shrink-0 outline-none bg-[#dddddd] dark:bg-[#2d2d2d] md:w-42.5 md:h-34 md:p-5 lg:w-45 lg:h-38 lg:p-6"
            >
              <span />
            ...
            {visibleAreas.map((a) => {
              const areaTableCount =
                tables?.filter((t) => (t.areaId?._id || t.areaId) === a._id).length || 0;
              ...
              return (
                <div key={a._id} className="relative shrink-0">
                  ...
                  <button
                    onClick={() => {
                      if (activeAction !== "delete-area") setSelectedArea(a._id);
                    }}
                    className="flex flex-col justify-between w-40 h-32.5 p-4 rounded-2xl shrink-0 outline-none bg-[#fef3b0] text-[#121212] md:w-42.5 md:h-34 md:p-5 lg:w-45 lg:h-38 lg:p-6"
                  >
                    <span />
